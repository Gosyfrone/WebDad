...
## Messages vocaux (16/06/2026)

- **Réutilisation des deux chemins média existants, zéro nouveau service.** Un vocal = un média audio. En **DM** il emprunte le chemin pièce jointe chiffrée (`uploadEncryptedMedia` → `/media/encrypted`, serveur aveugle, métadonnées dans l'enveloppe E2EE) → **aucun changement backend messagerie**. En **post** il emprunte le chemin média en clair (`uploadMedia` → `/media`, `MediaRef`).
- **« Front fait foi » pour le type audio en post.** La détection magic-bytes du conteneur WebM/Opus (produit par `MediaRecorder`) est ambiguë (souvent reconnue `video/webm`). Plutôt que de dépendre de la nature retournée par le serveur, le front impose `type:'audio'` sur le `MediaRef` ; le `media-service` n'a qu'à **accepter** les octets (allowlist élargie aux MIMEs audio, dont `audio/mp4`/`audio/x-m4a` pour Safari, sinon 415). Robuste cross-navigateur, sans heuristique serveur fragile.
- **Backend rétrocompatible.** `media-service` : `KindAudio` + cap dédié `MaxAudioBytes` (`MEDIA_MAX_AUDIO_BYTES`, 5 Mo). `post-service` : `MediaRef.Type` `oneof=image video audio` + ajout de `"audio"` aux enums `$jsonSchema` (posts/comments). Ajouter une valeur à un enum **optionnel** est rétrocompatible → **pas de backfill** (les docs image/video restent valides), contrairement aux champs contraints de la règle 5b.
- **Plafond 60 s appliqué côté client.** Un DM est E2EE : le serveur ne peut pas décoder l'audio pour vérifier la durée. Le recorder auto-stoppe à 60 s ; le **cap de taille du media-service reste la vraie barrière serveur**.
- **UX double mode, détectée via `pointer: coarse`.** PC : clic micro → enregistrement avec onde + chrono couvrant la zone de saisie → flèche (envoyer) / corbeille (annuler). Mobile : appui maintenu → glissé vers la corbeille (annuler) / relâcher (envoyer). Permission micro demandée **au premier `start`**, jamais avant.
- **Différence d'envoi DM vs post (demande explicite).** DM = envoi **direct** à la validation. Post = ajout au **brouillon supprimable**, publié seulement au submit.
- **Lecteur partagé** (`VoiceMessage`) : play/pause, onde **cliquable/glissable** (seek), **vitesse cyclique 1x→1.5x→2x**, contournement du bug de durée `Infinity` des blobs WebM sous Chrome.
