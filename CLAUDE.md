...
> **Session freshness:** current work log updated in `CHANGELOG.md` on 24/06/2026 (feat(onboarding/help) — **didacticiel guidé + page d'aide + bouton « lampe »**. Moteur **custom maison** (zéro dépendance, clair/sombre, i18n maison) ; auto-start à la 1ʳᵉ connexion via flag **backend** `tutorial_done *bool` sur `profil-service` (réutilise `PATCH/GET /profils/me`, aucune route nouvelle, **pas de backfill** car champ optionnel non-required/enum → prod non rejetée ; default explicite false à la Create). Front : `lib/tutorial-steps.ts` (+ `computeTooltipPosition` testé), `components/tutorial/{tutorial-provider,tutorial-tooltip}`, `components/help/{help-button,help-content}`, page `/aide`. Cibles `data-tour` posées sur sidebar (desktop) + tab bar/cloche (mobile), tooltip choisit l'instance visible ; clic-cible = avancer. Lampe : fixe bas-droite PC, à droite de la cloche mobile, masquée pour le visiteur. i18n **12 locales** (`tutorial.*`/`help.*`). Vérifs front : `tsc` OK, `vitest` **961 OK** (+10 nouveaux), ESLint propre. **⚠️ À lancer côté dev (Go absent de la session)** : `go test ./...` profil-service (+`TestPlanUpdate_TutorialDone`, `TestTutorialDoneOf`), **`make swagger`** (body `UpdateProfilRequest` modifié), `graphify update .`. Commit suggéré : `feat(onboarding): guided tutorial, help page and help launcher`). Entrée précédente 24/06/2026 (fix(register) — case CGU directement cochable, suppression du gate de lecture ; front-only).
