...
| Messagerie privée chiffrée (E2EE) | Secondary | 🟢 De bout en bout (back + UX). DM/groupes admin-proof, communautés hybrides (clé serveur). UI `/messages` deux volets (cf. §5). État lu/non-lu client (localStorage, par appareil), aperçu dernier message, épinglage/suppression côté user. Clé d'identité par appareil (IndexedDB). TODO : accusés de lecture multi-appareil, pièces jointes. |
...
- **MESSAGERIE — COMPLÈTE (back + UX).** message-service 🟢 (DM + groupes + communautés +
  ...
  conversation affiche un bandeau « clé indisponible » et désactive lecture/écriture). **Perspectives** :
  aperçu du dernier message dans la liste (nécessiterait de déchiffrer le dernier message par conv au
  chargement), accusés de lecture, pièces jointes, rotation de clé (forward secrecy), transfert
  d'ownership, backup passphrase (login multi-appareil), coffre séparé pour `content_key`.
