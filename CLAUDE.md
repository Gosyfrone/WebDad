...
*Last updated: 04/06/2026 — feat(frontend #81) : **dark mode refait dans la direction graphique de la refonte** (glassmorphism violet quasi-noir, dégradé de marque violet→indigo→cyan calé sur le logo "B"). La refonte #79 codait les couleurs en dur (≈230 occ.) → **tokenisation** : variables CSS `globals.css` (light = pixel-identique, dark = violet sombre) + classes `@layer components` (`.bg-page`/`.panel`/`.glass`/`.glass-column`/`.brand-text`), textes `slate-*` → tokens sémantiques, long tail en `dark:`. **Interrupteur clair/sombre ajouté sur PC** (`SidebarLeft`, au-dessus de la card user, même composant que le tiroir mobile) + **variante compacte flottante translucide bas-gauche sur login/register** (`FloatingThemeToggle`, layout `(auth)`). 24 fichiers touchés (layouts, sidebars, feed, profil, composer, chrome mobile, 6 pages stub, login/register). `tsc` + `next build` OK (19 routes, 0 warning). (précédemment : feat(profil-service) routes lecture/édition).*
