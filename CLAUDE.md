...
*Last updated: 03/06/2026 — feat(auth) : refresh tokens. Back : access (15m) + refresh (24h) opaque haché + rotation + révocation (`/auth/refresh`, `/auth/logout`), middleware `code: token_expired`. Front : access en localStorage, refresh en cookie httpOnly (BFF `/api/auth/{refresh,logout}`), `lib/auth-client` (`apiFetch` refresh single-flight sur 401 + `logout`), garde de session (`middleware.ts`), boutons déconnexion câblés. tsc + lint OK. (précédemment : branchement auth login/register + provisioning user)*
