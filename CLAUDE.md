...
| Profil Service | 🟡 WIP (squelette) | MongoDB | Squelette complet calqué sur post-service (config/database/models/repository/service/handler/middleware). **Autonome** : schéma embarqué au boot (`EnsureSchema`, collection `profiles` + validateur `$jsonSchema` + index unique `user_id` + seed admin upsert) → `profil-init.js` supprimé + mount compose retiré. **Propriétaire des champs décoratifs/perso UNIQUEMENT** : `user_id`, `display_name`, `bio`, `avatar_url`, `banner_url`, `website`, `location`, `birth_date` (date, opt.), `gender` (enum `male`/`female`, opt.), timestamps (pas de username ni compteurs → user-service, une seule source de vérité). Routes câblées (chaîne handler→service→repository) mais **stubs `501`** sauf `/health` : `GET /profils/:userId`, `GET/PATCH /profils/me`, `POST /profils`, `DELETE /profils/:userId` (admin). Middleware JWT (validation locale, secret partagé). Tests Go (enregistrement routes + health 200 + stub 501 + 401 sans token). TODO : implémenter repository/service (provisioning lazy + édition), brancher le front, upload avatar/bannière |
...
| User profile | Primary | 🟡 UI faite (consultation : bannière/avatar/bio/compteurs/onglets + édition popup nom/bio + photo/bannière via sélecteur de fichier avec aperçu local). **Back : squelette profil-service** (schéma autonome + routes stubs 501 ; propriétaire de display_name/bio/avatar/banner/website/location). Reste : implémenter repository/service + brancher le front (lecture agrégée user+profil+post, édition = 1 PATCH /profils/me) + upload réel |
...
- **À FAIRE — implémenter le profil-service (repository/service).** Le squelette est posé (routes
  câblées mais **stubs `501`**, schéma autonome, tests d'enregistrement). Reste à coder : lecture
  `GET /profils/:userId`, **provisioning paresseux** sur `GET /profils/me` (upsert depuis les
  claims JWT, comme user-service), `PATCH /profils/me`, `POST`/`DELETE`. Puis brancher le front
  (agrégation lecture user+profil+post — cf. décision §5) et l'upload avatar/bannière.
