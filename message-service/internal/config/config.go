// Package config lit la configuration du message-service depuis les variables
// d'environnement. Aucune valeur en dur : l'URI Mongo est construite à partir
// du .env (même convention que les autres services Mongo, cf. post-service).
package config
...
// l'override compose (MONGO_HOST=mongo-message) reste prioritaire.
func Load() *Config {
	_ = godotenv.Load(".env")
	_ = godotenv.Load("../.env")
