// Package realtime gère la diffusion temps réel des messages via WebSocket.
//
// Modèle : une connexion WS par utilisateur (authentifiée par JWT). Le hub
// indexe les connexions par user_id. Quand un message est posté, le service
// fournit les ids des membres de la conversation et le hub pousse l'événement
// (le message CHIFFRÉ — le serveur ne déchiffre rien) à chaque membre connecté.
package realtime
