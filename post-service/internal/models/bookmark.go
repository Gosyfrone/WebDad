...
// BookmarkCollection — une « collection » de signets, propre à un utilisateur
...
type BookmarkCollection struct {
	...
	// IsDefault : collection « par défaut » de l'utilisateur (une seule par compte,
...
// Bookmark — un post rangé dans une collection (collection `bookmarks`).
// Many-to-many : un même post peut figurer dans plusieurs collections, soit un
...
// un document par user, `user_id` unique). Mémorise la dernière collection
...
// serveur redemande la collection (« ouverture de rafale »).
...
// (range dans la dernière collection, ou propose un choix). Fourni → ajout/retrait
// explicite dans cette collection précise (appui long / sélecteur).
