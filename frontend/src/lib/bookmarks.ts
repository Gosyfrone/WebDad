 ...
 *
 * Les signets sont organisés en **playlists** (many-to-many : un post peut être
 * rangé dans plusieurs playlists). Tous les appels passent par `apiFetch`
 ...
 * automatiquement dans la dernière playlist si l'utilisateur enchaîne dans la
 * fenêtre glissante (statut `filed`) ; sinon le serveur renvoie `needs_choice`
 * et le front ouvre le sélecteur de playlist (cf. CLAUDE.md §5).
 *
 * Les vues qui renvoient des posts (« Tous mes signets », contenu d'une
 * playlist) réutilisent le mapper enrichi `mapPosts` de `lib/posts.ts`.
 */
...
/** Une playlist de signets. */
export interface BookmarkCollection {
  id: string
  name: string
  /** Playlist par défaut : toujours en tête, non supprimable / non renommable. */
  isDefault: boolean
  itemsCount: number
  createdAt: string
}
 ...
 *    (`collections` = playlists existantes proposées), rien n'est rangé.
 */
...
/** Playlists de signets de l'utilisateur courant (avec compteur d'items). */
...
/** Supprime une playlist (et ses signets). */
...
/** Range explicitement un post dans une playlist (sélecteur / appui long). */
...
/** Retire un post d'une playlist précise. */
...
/** Dé-signe : retire un post de TOUTES les playlists. */
...
/** Ids des playlists de l'utilisateur contenant ce post (coche le sélecteur). */
...
/** Posts d'une playlist (du plus récemment rangé au plus ancien), paginés. */
