...
| Frontend | 🟡 WIP | — | Next.js 14, X-style responsive layout, refresh-token auth, business clients over `apiFetch`. Wired: feed/posts, trends/search/Explorer, bookmarks, hydrated profile + privacy, **combobox de nationalité via `/api/countries` + badges genre/drapeau**, E2EE messaging + attachments, notifications, **i18n 12 langues avec préférence persistée par compte**, traduction automatique prudente des posts/commentaires, dark mode, legal pages, muted words, @mentions, media, OAuth Google, onboarding admin/OAuth et vue visiteur. **TODO:** post edit. |
