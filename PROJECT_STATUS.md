...
| Login with Google (OAuth/OIDC) | Secondary | 🟢 End-to-end. BFF `GET /api/auth/oauth/[provider]` (proxy URL) + `POST /api/auth/oauth/[provider]` (exchange code→cookie+token) + callback page `/(auth)/auth/callback/[provider]` → URL `/auth/callback/google` (Suspense, `useSearchParams`, `provisionUser`, `setRefreshCookie`). Bouton Google sur login + register (loading state, error). Google Cloud OAuth client configuré (credentials dans `auth-service/.env`). **TODO:** new-user profil provisioning (username/gender/birthDate). *(Microsoft retiré sur demande — front + back, cf. CHANGELOG 10/06/2026.)* |
