...
| Login with Google (OAuth/OIDC) | Secondary | 🟡 Back-end done in auth-service: `/auth/oauth/{provider}/{url,exchange}`, server-side code→tokens + ID token verification (JWKS/issuer/audience), email-based find-or-create, generic registry (Microsoft = config entry). **TODO:** front (callback page `/auth/callback/google` + BFF), new OAuth-user username/profil provisioning, Microsoft credentials. |
