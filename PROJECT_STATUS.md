...
| Login with Google (OAuth/OIDC) | Secondary | 🟡 Back-end done. **Front done**: BFF `GET /api/auth/oauth/[provider]` (proxy URL) + `POST /api/auth/oauth/[provider]` (exchange code→cookie+token) + callback page `/(auth)/callback/[provider]` (Suspense, `useSearchParams`, `provisionUser`, `setRefreshCookie`). Bouton Google branché sur login + register (loading state, error). **TODO:** Microsoft credentials, new-user profil provisioning (username/gender/birthDate). |
