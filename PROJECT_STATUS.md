...
| Post | 🟢 OK (v1) | MongoDB | Autonomous. CRUD posts with **profile-visibility-filtered reads**, **hashtags + filtered feed + trends**, reposts/quotes, profile pin (hidden in feeds), likes, **`GET /posts/liked?author_id=` (likes-visibility-gated)**, 2-level threaded comments, denormalized int32 counters, **media `[]MediaRef` (cap 4)**, **bookmark collections + burst model**. **TODO:** post edit front (back ready). |
