...
|---|---|---|---|
| Auth | 🟡 WIP | PostgreSQL | `/auth/{register,login,refresh,logout,validate,verify-email/confirm,verify-email/request,password/forgot,password/reset}` + `/auth/oauth/{provider}/{url,exchange}` + `/health`. JWT HS256 + bcrypt, access 15m + refresh 24h (rotation, revoke). **OIDC Login with Google** (code→tokens server-side, ID token verified via JWKS/issuer/audience; generic provider registry). **Microsoft multi-tenant ready** (`common`: manual issuer check + `tid` consistency; `email_verified` absent → assumed verified). **Blocage dur login non-vérifié** (`403 email_not_verified`) ; register envoie le mail de vérif (best-effort) + tokens vérif opaques `account_tokens` (24h, usage unique, anti-énum sur le renvoi). **`verify-email/confirm` = auto-login** : confirme le token *et* émet une session (access + refresh, comme login ; garde compte désactivé). **Reset password** (Phase 2) : forgot anti-énum (toujours 200) + reset token 1h usage unique → bcrypt + `email_verified=true` + **révocation de toutes les sessions** (`DELETE refresh_tokens`). Autonomous schema (idempotent ALTER: `provider`/`provider_subject`, password nullable) + optional admin seed. **TODO:** Go tests, refresh-reuse detection, OAuth new-user provisioning, Microsoft credentials (Entra app registration), rate-limiting (Phase 3). |
...
| Login with Google (OAuth/OIDC) | Secondary | 🟡 Back-end done. **Front done**: BFF `GET /api/auth/oauth/[provider]` (proxy URL) + `POST /api/auth/oauth/[provider]` (exchange code→cookie+token) + callback page `/(auth)/callback/[provider]` (Suspense, `useSearchParams`, `provisionUser`, `setRefreshCookie`). Boutons Google + Microsoft branchés sur login + register (loading state, error). Microsoft multi-tenant côté back (issuer vérifié manuellement, `email_verified` absent toléré). **TODO:** Microsoft credentials (Entra: "any org + personal accounts", redirect `http://localhost:3000/auth/callback/microsoft`), new-user profil provisioning (username/gender/birthDate). |
