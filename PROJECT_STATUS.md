...
| Search / Explorer (accounts + hashtags) | Secondary | 🟢 user + profil search by `@username`, plain username, or display_name; direct `#hashtag` navigation; hashtag page + right-column dropdowns suggest trends then profiles; recent clicked searches can be cleared globally or one by one; "Who to follow"; per-account local search history. **TODO:** post full-text search. |
...
| Hashtags & trends | Secondary | 🟢 End-to-end: hashtags extracted server-side on posts, clickable render, `?hashtag=` results view (`À la une`/`Récent`/`Média`), and dynamic Top 5 trends from visible posts. |
